...
export class WebhookSubscriptionDetailComponent extends AppComponentBase implements OnInit {
  ...
  toggleActivity(): void {
    let message = this.subscription.isActive ? this.l('DeactivateSubscriptionWarningMessage') : this.l('ActivateSubscriptionWarningMessage');
  ...
  resend(id: string): void {
    this.message.confirm(this.l('WebhookEventWillBeSendWithSameParameters'), this.l('AreYouSure'), (isConfirmed) => {
      if (isConfirmed) {
