...
export class HostSettingsComponent extends AppComponentBase implements OnInit {
  ...
  loadHostSettings(): void {
    ...
    self._hostSettingService.getAllSettings().subscribe((setting) => {
      ...
      this.wsFederationClaimMappings = this.hostSettings.externalLoginProviderSettings.openIdConnectClaimsMapping.map((item) => {
        return {
          key: item.key,
          value: item.claim,
        };
  ...
  mapClaims(): void {
    if (this.wsFederationClaimsMappingManager) {
      this.hostSettings.externalLoginProviderSettings.wsFederationClaimsMapping = this.wsFederationClaimsMappingManager.getItems().map(
        (item) =>
          new JsonClaimMapDto({
            key: item.key,
            claim: item.value,
