...
@Component({
  ...
  template: ` <div *ngIf="delegations?.length" class="topbar-item active-user-delegations">
    <div [class]="customStyle">
      <select class="form-control form-control-sm" (change)="switchToDelegatedUser($event)" [(ngModel)]="selectedUserDelegationId">
        ...
        <option *ngFor="let delegation of delegations" [value]="delegation.id" [attr.data-username]="delegation.username"
          >{{ delegation.username }} ({{ 'ExpiresAt' | localize: (delegation.endTime | momentFormat: 'L LT') }})</option
        >
      </select>
    </div>
  </div>`,
...
export class ActiveDelegatedUsersComboComponent extends AppComponentBase implements OnInit {
  ...
  constructor(private _userDelegationService: UserDelegationServiceProxy, private _impersonationService: ImpersonationService, injector: Injector) {
    super(injector);
  }
  ...
  switchToDelegatedUser($event): void {
    ...
    this.message.confirm(this.l('SwitchToDelegatedUserWarningMessage', username), this.l('AreYouSure'), (isConfirmed) => {
      if (isConfirmed) {
