...
export class ChatBarComponent extends AppComponentBase implements OnInit, AfterViewInit {
  ...
  uploadImage(data: { files: File }): void {
    ...
    this._httpClient
      ...
      .subscribe((response) => {
        this.chatMessage =
          '[image]{"id":"' + response.result.id + '", "name":"' + response.result.name + '", "contentType":"' + response.result.contentType + '"}';
        this.sendMessage();
  ...
  uploadFile(data: { files: File }): void {
    ...
    this._httpClient
      ...
      .subscribe((response) => {
        this.chatMessage =
          '[file]{"id":"' + response.result.id + '", "name":"' + response.result.name + '", "contentType":"' + response.result.contentType + '"}';
        this.sendMessage();
  ...
  getFilteredFriends(state: FriendshipState, userNameFilter: string): FriendDto[] {
    const foundFriends = _.filter(
      ...
      (friend) =>
        friend.state === state &&
        this.getShownUserName(friend.friendTenancyName, friend.friendUserName).toLocaleLowerCase().indexOf(userNameFilter.toLocaleLowerCase()) >= 0
    );
  ...
  getFixedMessageTime(messageTime: moment.Moment): string {
    return moment(messageTime)
      .add(-1 * this.serverClientTimeDifference, 'seconds')
    ...
    function onMessageReceived(message) {
      ...
      if (message.side === ChatSide.Receiver) {
        ...
        if (
          ...
          self.notify.info(abp.utils.formatString('{0}: {1}', user.friendUserName, abp.utils.truncateString(message.message, 100)), null, {
            onclick() {
  ...
  init(): void {
    ...
    this.interTenantChatAllowed =
      this.feature.isEnabled('App.ChatFeature.TenantToTenant') || this.feature.isEnabled('App.ChatFeature.TenantToHost') || !this.appSession.tenant;
  }
}
