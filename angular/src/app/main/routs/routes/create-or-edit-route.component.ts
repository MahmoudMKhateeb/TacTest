...
import { RoutesServiceProxy, CreateOrEditRouteDto, RouteRoutTypeLookupTableDto } from '@shared/service-proxies/service-proxies';
...
export class CreateOrEditRouteComponent extends AppComponentBase implements OnInit {
  ...
  constructor(injector: Injector, private _activatedRoute: ActivatedRoute, private _routesServiceProxy: RoutesServiceProxy, private _router: Router) {
    super(injector);
  }
  ...
  private saveInternal(): Observable<number> {
    ...
    return this._routesServiceProxy.createOrEdit(this.route).pipe(
      finalize(() => {
        this.saving = false;
