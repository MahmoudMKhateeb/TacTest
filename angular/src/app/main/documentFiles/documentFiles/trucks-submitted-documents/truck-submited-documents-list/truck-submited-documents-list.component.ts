...
@Component({
  ...
  templateUrl: './truck-submited-documents-list.component.html',
...
export class TruckSubmitedDocumentsListComponent extends AppComponentBase implements OnInit {
    ...
    this.dataSource.store = new CustomStore({
      load(loadOptions: LoadOptions) {
        return self._documentFilesServiceProxy
          .getAllTrucksSubmittedDocuments(JSON.stringify(loadOptions), undefined)
          .toPromise()
          .then((response) => {
            return {
              data: response.data,
  ...
  getDocumentFilesByTruckId(truckId: any) {
    let self = this;
    ...
    this.dataSource.store = new CustomStore({
      load(loadOptions: LoadOptions) {
        return self._documentFilesServiceProxy
          .getAllTrucksSubmittedDocuments(JSON.stringify(loadOptions), truckId)
          .toPromise()
          .then((response) => {
            return {
              data: response.data,
