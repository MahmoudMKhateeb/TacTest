...
@Component({
  ...
  styleUrls: ['./invoices-dedicated.component.css'],
  animations: [appModuleAnimation()],
})
export class InvoicesDedicatedComponent extends AppComponentBase implements OnInit {
  @ViewChild('InvoiceDetailsModel', { static: true }) InvoiceDetailsModel: InvoiceTenantItemsDetailsComponent;
  @ViewChild('InvoiceDedicatedModal', { static: true }) InvoiceDedicatedModal: InvoiceDedicatedModalComponent;
  ...
  ngOnInit() {
    this.items = this._enumToArrayPipeService.transform(InvoiceAccountType).map((item) => {
      return {
        ...
        icon: '',
        command: () => {
          this.InvoiceDedicatedModal.show(Number(item.key));
        },
      };
    });
  ...
  getAllInvoices() {
    ...
    this.dataSource.store = new CustomStore({
      load(loadOptions: LoadOptions) {
        return self._DedicatedDynamiceInvoicesServiceProxy
      ...
      remove: (data) => {
        return this._DedicatedDynamiceInvoicesServiceProxy
          .delete(data.id)
          .toPromise()
          .then((res) => {
  ...
  editRow(item, isView = false) {
    ...
    this.InvoiceDedicatedModal.show(item.invoiceAccountType, item.id, item.tenantName, isView);
    // return false;
  }

  dynamicInvoiceOnDemand(item) {
    if (isNotNullOrUndefined(item.invoiceNumber) && item.invoiceNumber !== 0) {
      Swal.fire({
