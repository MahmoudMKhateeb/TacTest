...
namespace TACHYON.Tracking
{
    ...
    public class ShippingRequestPointWorkFlowProvider : TACHYONDomainServiceBase,
        IWorkFlowProvider<WorkflowTransaction<PointTransactionArgs,RoutePointStatus>,PointTransactionArgs,RoutePointStatus>
    {
        public List<WorkFlow<WorkflowTransaction<PointTransactionArgs, RoutePointStatus>>> Flows { get; set; }
            ...
            IRepository<AdditionalStepTransition, long> additionalStepTransition)
        {
            ...
            Flows = new List<WorkFlow<WorkflowTransaction<PointTransactionArgs, RoutePointStatus>>>
            {
                // Pick up workflow 
                new()
                {
                    Version = WorkflowVersionConst.PickupPointWorkflowVersion,
                ...
                new()
                {
                    Version = WorkflowVersionConst.DropOffWithDeliveryNotePointWorkflowVersion,
                    Transactions = new List<WorkflowTransaction<PointTransactionArgs, RoutePointStatus>>
                    {
                        new WorkflowTransaction<PointTransactionArgs, RoutePointStatus>
                        {
                ...
                new()
                {
                    ...
                    Transactions = new List<WorkflowTransaction<PointTransactionArgs, RoutePointStatus>>
                    {
                        new WorkflowTransaction<PointTransactionArgs, RoutePointStatus>()
                        {
                            Action = WorkFlowActionConst.PickupConfirmDelivery,
                            FromStatus = RoutePointStatus.StandBy,
                            ToStatus = RoutePointStatus.FinishLoading,
                            Func = PickupConfirmDelivery,
                            Name = "ConfirmPickupDelivery",
                            Permissions = new List<string>(),
                            Features = new List<string>()
                        },
                    }
                ...
                new()
                {
                    Version = WorkflowVersionConst.DropOffHomeDeliveryWorkflowVersion,
                    Transactions = new List<WorkflowTransaction<PointTransactionArgs, RoutePointStatus>>
                    {
                        new WorkflowTransaction<PointTransactionArgs, RoutePointStatus>()
                        {
                ...
                new()
                {
                    Version = WorkflowVersionConst.DropOffHomeDeliveryWithPodWorkflowVersion,
                    Transactions = new List<WorkflowTransaction<PointTransactionArgs, RoutePointStatus>>
                    {
                        new WorkflowTransaction<PointTransactionArgs, RoutePointStatus>()
                        {
                ...
                new()
                {
                    Version = WorkflowVersionConst.DropOffHomeDeliveryWithReceiverCodeWorkflowVersion,
                    Transactions = new List<WorkflowTransaction<PointTransactionArgs, RoutePointStatus>>
                    {
                        new WorkflowTransaction<PointTransactionArgs, RoutePointStatus>()
                        {
                ...
                new()
                {
                    Version = WorkflowVersionConst.DropOffPortsMovementWorkflowVersion,
                    Transactions = new List<WorkflowTransaction<PointTransactionArgs, RoutePointStatus>>
                    {
                        new()
                        {
        ...
        public async Task Start(ShippingRequestTripDriverStartInputDto Input)
        {
            ...
            {
                await Invoke(new PointTransactionArgs() { PointId = routeStart.Id, ForceDeliverModeEnabled = true },
                    WorkFlowActionConst.PickupConfirmDelivery);
            }
        ...
        private async Task<string> ArriveToLoadingLocation(PointTransactionArgs args)
        {
            ...
            point.Status = status;
            point.ShippingRequestTripFk.RoutePointStatus = status;
            if (point.ShippingRequestTripFk.ShippingRequestFk != null)
            {
        ...
        private async Task<string> FinishLoading(PointTransactionArgs args)
        {
            ...
            await SendSmsToReceivers(point.ShippingRequestTripId, point.ShippingRequestTripFk.WaybillNumber, point.ShippingRequestTripFk.ShippingRequestFk?.RouteTypeId ?? point.ShippingRequestTripFk.RouteType);
        ...
        private async Task<string> FinishOffLoadShipment(PointTransactionArgs args)
        {
            ...
            if (shippingRequest != null)
            {
                await _penaltyManager.ApplyDetentionPenalty
                (
                ...
                if (point.ShippingRequestTripFk.EndTripDate.HasValue)
                    await _penaltyManager.ApplyNotDeliveringAllDropsPenalty
                    (
                        shippingRequest.CarrierTenantId.Value,
                        shippingRequest.TenantId,
        ...
        private async Task<string> PickupConfirmDelivery(PointTransactionArgs arg)
        {
            var point = await _routPointRepository.GetAllIncluding(x => x.ShippingRequestTripFk)
                .SingleAsync(x => x.Id == arg.PointId);

            point.Status = RoutePointStatus.FinishLoading;
            point.ShippingRequestTripFk.RoutePointStatus = RoutePointStatus.FinishLoading;
            point.IsComplete = true;
            point.EndTime = Clock.Now;
            point.ActualPickupOrDeliveryDate = Clock.Now;
            point.ShippingRequestTripFk.ActualPickupDate = Clock.Now;
            point.CanGoToNextLocation = true;
            // no need to check arrival time (there is no penalty here)
        ...
        private async Task<ShippingRequestTrip> CheckIfCanAccepted(int tripId)
        {
            ...
            var trip = await _shippingRequestTripRepository
                            ...
                            .Where(x => x.Id == tripId)
                            .Where(x => (x.ShippingRequestFk.CarrierTenantId.HasValue || x.CarrierTenantId.HasValue) && x.ShippingRequestFk.Status != ShippingRequestStatus.Cancled && x.AssignedDriverUserId.HasValue)
                            .WhereIf(!currentUser.TenantId.HasValue || await _featureChecker.IsEnabledAsync(AppFeatures.TachyonDealer), x => true)
                            .WhereIf(currentUser.TenantId.HasValue && await _featureChecker.IsEnabledAsync(AppFeatures.Carrier), x => x.ShippingRequestFk.CarrierTenantId == currentUser.TenantId.Value || x.CarrierTenantId == currentUser.TenantId.Value)
        ...
        private async Task<ShippingRequestTrip> CheckIfCanStartTrip(int tripId)
        {
            ...
            return await _shippingRequestTripRepository
                ...
                .Where
                (
                    x => x.Status == ShippingRequestTripStatus.New &&
                         x.DriverStatus == ShippingRequestTripDriverStatus.Accepted
                )
                .WhereIf(!currentUser.TenantId.HasValue || await _featureChecker.IsEnabledAsync(AppFeatures.TachyonDealer), x => true)
                .WhereIf
                (
                    currentUser.TenantId.HasValue && await _featureChecker.IsEnabledAsync(AppFeatures.Carrier),
                    x => x.ShippingRequestFk.CarrierTenantId == currentUser.TenantId.Value || x.CarrierTenantId == currentUser.TenantId.Value
                )
                .WhereIf(currentUser.IsDriver, x => x.AssignedDriverUserId == currentUser.Id)
                .FirstOrDefaultAsync();
        }
        ...
        private async Task<RoutPoint> GetPickUpPointToStart(int tripId)
        {
            ...
            var pickupPoint = await _routPointRepository.GetAll().Include(x => x.FacilityFk)
                .OrderBy(x=> x.PointOrder ?? x.Id).FirstOrDefaultAsync(x=> x.ShippingRequestTripId == tripId && x.PickingType == PickingType.Pickup);
            if (pickupPoint is not { }) throw new UserFriendlyException(L("PickupPointNotFound")); 
        ...
        private async Task<RoutPoint> GetPointForInvoke(long pointId)
        {
            ...
            return await _routPointRepository
                ...
                .Where(x => x.IsResolve && !x.IsComplete)
                .WhereIf(!currentUser.TenantId.HasValue || await _featureChecker.IsEnabledAsync(AppFeatures.TachyonDealer), x => true)
                .WhereIf
                (
                    currentUser.TenantId.HasValue && await _featureChecker.IsEnabledAsync(AppFeatures.Carrier),
                    x => x.ShippingRequestTripFk.ShippingRequestFk.CarrierTenantId == currentUser.TenantId.Value ||
                         x.ShippingRequestTripFk.CarrierTenantId == currentUser.TenantId.Value
                )
        ...
        private async Task HandlePointDelivery(RoutPoint point, RoutePointStatus pointStatus)
        {
            bool IsMultipleDrops(ShippingRequestTrip shippingRequestTrip)
            {
                ...
                if (shippingRequestTrip.RouteType != null)
                {
                    return shippingRequestTrip.RouteType == ShippingRequestRouteType.MultipleDrops;
                }
                if (shippingRequestTrip.ShippingRequestFk != null)
                {
                    return shippingRequestTrip.ShippingRequestFk.RouteTypeId == ShippingRequestRouteType.MultipleDrops;
                }
            ...
            bool IsSingleDrop(ShippingRequestTrip shippingRequestTrip)
            {

                if (shippingRequestTrip.RouteType != null)
                {
                    return shippingRequestTrip.RouteType == ShippingRequestRouteType.SingleDrop;
                }
                if (shippingRequestTrip.ShippingRequestFk != null)
                {
                    return shippingRequestTrip.ShippingRequestFk.RouteTypeId == ShippingRequestRouteType.SingleDrop;
                }
        ...
        private async Task SendSmsToReceiver(RoutPoint point, long? tripWaybillNumber, ShippingRequestRouteType? routType)
        {
            ...
            if (point.ShippingRequestTripFk.ShippingRequestFk != null && point.ShippingRequestTripFk.ShippingRequestFk.IsSaas())
            {
                message = L(TACHYONConsts.SMSSaasShippingRequestReceiverCode, waybillNumber, point.Code);

            }
            else if (point.ShippingRequestTripFk.CarrierTenantId == point.ShippingRequestTripFk.ShipperTenantId) // saas trip
            {
                message = L(TACHYONConsts.SMSSaasShippingRequestReceiverCode, waybillNumber, point.Code);
            }
        ...
        {
            if (_shippingRequestTripRepository.GetAll().Where(x=> x.ShippingRequestId == trip.ShippingRequestId ).All(x =>
