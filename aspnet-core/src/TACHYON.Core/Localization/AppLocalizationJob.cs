...
namespace TACHYON.Localization
{
    public class AppLocalizationJob : BackgroundJob<string>, ITransientDependency
    {
        ...
        public AppLocalizationJob(IRepository<AppLocalization> appLocalizationRepository,
        ...
        private void Restore()
        {
            ...
            var master = LoadKeys($"{directory}/TACHYON.xml", "en")
                .Select(x => new AppLocalization { MasterKey = x.Key, MasterValue = x.Value }).ToList();


            var data = master.Where(m => !CurrentMaster.Any(c => c.MasterKey.ToLower() == m.MasterKey.ToLower()))
                .ToList();
            ...
            foreach (var lang in data)
            {
                ...
                {
                    lang.Translations = Translations.Where(x => x.Key.ToLower() == lang.MasterKey.ToLower()).Select(x =>
                        new AppLocalizationTranslation() { Language = x.Language, Value = x.Value }).ToList();
        ...
        private IEnumerable<TranslationDto> LoadKeys(string xmlPath, string lang)
        {
            ...
            return doc.Descendants("text").Select(x =>
                new TranslationDto() { Key = x.Attribute("name").Value, Value = x.Value, Language = lang });
        }
