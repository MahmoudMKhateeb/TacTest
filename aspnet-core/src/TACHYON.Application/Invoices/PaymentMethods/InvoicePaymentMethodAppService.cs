...
namespace TACHYON.Invoices.PaymentMethods
{
    ...
    public class InvoicePaymentMethodAppService : TACHYONAppServiceBase, IInvoicePaymentMethodAppService
    {
        ...
        public ListResultDto<InvoicePaymentMethodListDto> GetAll(FilterInput Input)
        {
            var query = _invoicePaymentMethodRepository
                ...
                .WhereIf(!string.IsNullOrEmpty(Input.Filter),
                    x => x.DisplayName.ToLower().Contains(Input.Filter.Trim().ToLower()))
                .OrderBy(Input.Sorting ?? "id");

            return new ListResultDto<InvoicePaymentMethodListDto>(
                ObjectMapper.Map<List<InvoicePaymentMethodListDto>>(query));
        }
        ...
        {
            if (_invoicePaymentMethodRepository.GetAll().Any(x =>
                    x.Id != input.Id && x.DisplayName.ToLower() == input.DisplayName.Trim().ToLower()))
            {
        ...
        {
            input.Id = await _invoicePaymentMethodRepository.InsertAndGetIdAsync(
                ObjectMapper.Map<InvoicePaymentMethod>(input));
            return input;
        }
