...
namespace TACHYON.DemoUiComponents
{
    ...
    public class DemoUiComponentsAppService : TACHYONAppServiceBase, IDemoUiComponentsAppService
    {
        #region date & time pickers

        public DateToStringOutput SendAndGetDate(DateTime? date)
        {
            return new DateToStringOutput { DateString = date?.ToString("d") };
        }

        public DateToStringOutput SendAndGetDateTime(DateTime? date)
        {
            return new DateToStringOutput { DateString = date?.ToString("g") };
        }

        public DateToStringOutput SendAndGetDateRange(DateTime? startDate, DateTime? endDate)
        {
            return new DateToStringOutput { DateString = startDate?.ToString("d") + " - " + endDate?.ToString("d") };
        }
