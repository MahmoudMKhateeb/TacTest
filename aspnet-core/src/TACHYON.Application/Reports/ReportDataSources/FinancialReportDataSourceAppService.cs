    ...
    public async Task<IEnumerable<FinancialItem>> GetAll(Guid reportId)
    {
        ...
        var financialItems = await (from trip in tripsQuery
            ...
            .Select(x => new FinancialItem
            {
                ...
                RequestId = x.Trip.ShippingRequestId.HasValue ? x.Trip.ShippingRequestFk.ReferenceNumber : EmptyField,
                TransportType = x.Trip.ShippingRequestId.HasValue ? x.Trip.ShippingRequestFk.TransportTypeFk.Translations.FirstOrDefault(t=> t.Language.Contains(CultureInfo.CurrentUICulture.Name)).TranslatedDisplayName :
                    x.Trip.AssignedTruckFk.TransportTypeFk.Translations.FirstOrDefault(t=> t.Language.Contains(CultureInfo.CurrentUICulture.Name)).TranslatedDisplayName,
                TruckType = x.Trip.ShippingRequestId.HasValue ? x.Trip.ShippingRequestFk.TrucksTypeFk.Translations.FirstOrDefault(t=> t.Language.Contains(CultureInfo.CurrentUICulture.Name)).TranslatedDisplayName :
                    x.Trip.AssignedTruckFk.TrucksTypeFk.Translations.FirstOrDefault(t=> t.Language.Contains(CultureInfo.CurrentUICulture.Name)).TranslatedDisplayName,
                ...
                RequestCreationDate = x.Trip.ShippingRequestId.HasValue ? x.Trip.ShippingRequestFk.CreationTime.ToString("dd/MM/yyyy") : EmptyField,
                CarrierInvoiceNumber = x.CarrierInvoice != null ? x.CarrierInvoice.InvoiceFK.InvoiceNumber : null,
                CarrierInvoiceStatus = x.CarrierInvoice != null ? LocalizationSource.GetString(x.CarrierInvoice.InvoiceFK.Status.ToString()) : EmptyField,
                ShipperInvoiceNumber = x.ShipperInvoice != null ? x.ShipperInvoice.InvoiceFK.InvoiceNumber : null,
                ShipperInvoiceStatus = x.ShipperInvoice != null ? LocalizationSource.GetString(x.ShipperInvoice.InvoiceFK.Status.ToString()) : EmptyField,
                CostWithoutVat = x.Trip.SubTotalAmount, CostWithVat = x.Trip.TotalAmount,
                SellingWithoutVat = x.Trip.SubTotalAmountWithCommission, SellingWithVat = x.Trip.TotalAmountWithCommission,
                TachyonCommissionWithVat = x.Trip.VatAmountWithCommission, TachyonCommissionWithoutVat = x.Trip.CommissionAmount,
                ShipperInvoiceConfirmationDate = x.ShipperInvoice != null && x.ShipperInvoice.InvoiceFK.ConfirmationDate.HasValue ? 
                    x.ShipperInvoice.InvoiceFK.ConfirmationDate.Value.ToString("dd/MM/yyyy"): EmptyField,
                ShipperInvoiceIssuanceDate = x.ShipperInvoice != null ? x.ShipperInvoice.InvoiceFK.CreationTime.ToString("dd/MM/yyyy"): EmptyField,
            }).ToListAsync();
