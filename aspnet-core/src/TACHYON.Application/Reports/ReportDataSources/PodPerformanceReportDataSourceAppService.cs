       ...
       var tripsQuery =   _tripRepository.GetAll().AsNoTracking()
          ...
          var podReportItems = await (from trip in tripsQuery 
                  from invoiceTrip in _invoiceTripRepository.GetAll().Where( i=> trip.Id == i.TripId).DefaultIfEmpty() 
                   select new
                   {
                       Trip = trip, Invoice = invoiceTrip.InvoiceFK,
                       LastUploadedPodDate = trip.RoutPoints.Where(p=> p.PickingType == PickingType.Dropoff)
                           .SelectMany(p=> p.RoutPointStatusTransitions).Where(p=> !p.IsReset && p.Status == RoutePointStatus.DeliveryConfirmation)
               ...
               .Select(x=> new PodPerformanceItem
            {
                AccountManager = x.Trip.ShippingRequestId.HasValue ?
                    (isShipper ? x.Trip.ShippingRequestFk.Tenant.FinancialName : (isCarrier? x.Trip.ShippingRequestFk.CarrierTenantFk.FinancialName : EmptyField )) 
                    : (isShipper ? x.Trip.ShipperTenantFk.FinancialName : (isCarrier ? x.Trip.CarrierTenantFk.FinancialName : EmptyField )),
                BookingNumber = x.Trip.ShippingRequestId.HasValue ? x.Trip.ShippingRequestFk.ShipperInvoiceNo : EmptyField,
                ContainerNumber = string.IsNullOrEmpty(x.Trip.ContainerNumber) ? x.Trip.ContainerNumber : EmptyField,
                ...
                DriverName = x.Trip.AssignedDriverUserId.HasValue ? $"{x.Trip.AssignedDriverUserFk.Name} {x.Trip.AssignedDriverUserFk.Surname}" : EmptyField,
                InvoiceStatus = isShipper ? LocalizationSource.GetString((x.Trip.IsShipperHaveInvoice ? "InvoiceIssued" : "InvoiceNotIssued")) 
                    : (isCarrier ? LocalizationSource.GetString((x.Trip.IsCarrierHaveInvoice ? "InvoiceIssued" : "InvoiceNotIssued")) : EmptyField),
                ...
                ShippingType = x.Trip.ShippingRequestId.HasValue ? LocalizationSource.GetString(x.Trip.ShippingRequestFk.ShippingTypeId.ToString()) 
                    : (x.Trip.ShippingTypeId.HasValue ? LocalizationSource.GetString(x.Trip.ShippingTypeId.ToString()) : EmptyField),
                ...
                IsUploadedWithin7Days =  x.LastCompletedPointFinishOffloadingDate.AddDays(7) > x.LastUploadedPodDate
                    ? LocalizationSource.GetString("Yes") : LocalizationSource.GetString("No"),
            }).ToListAsync();
