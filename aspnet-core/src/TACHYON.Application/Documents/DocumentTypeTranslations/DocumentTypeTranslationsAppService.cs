...
namespace TACHYON.Documents.DocumentTypeTranslations
{
    ...
    public class DocumentTypeTranslationsAppService : TACHYONAppServiceBase, IDocumentTypeTranslationsAppService
    {
		 ...
		 private readonly IRepository<DocumentType,long> _lookup_documentTypeRepository;
		  ...
		  public DocumentTypeTranslationsAppService(IRepository<DocumentTypeTranslation> documentTypeTranslationRepository, IDocumentTypeTranslationsExcelExporter documentTypeTranslationsExcelExporter , IRepository<DocumentType, long> lookup_documentTypeRepository) 
		  {
		 ...
		 public async Task<PagedResultDto<GetDocumentTypeTranslationForViewDto>> GetAll(GetAllDocumentTypeTranslationsInput input)
         {
			
			var filteredDocumentTypeTranslations = _documentTypeTranslationRepository.GetAll()
						.Include( e => e.Core)
						.WhereIf(!string.IsNullOrWhiteSpace(input.Filter), e => false  || e.Name.Contains(input.Filter) || e.Language.Contains(input.Filter))
						.WhereIf(!string.IsNullOrWhiteSpace(input.NameFilter),  e => e.Name == input.NameFilter)
						.WhereIf(!string.IsNullOrWhiteSpace(input.LanguageFilter),  e => e.Language == input.LanguageFilter)
						.WhereIf(!string.IsNullOrWhiteSpace(input.DocumentTypeDisplayNameFilter), e => e.Core != null && e.Core.DisplayName == input.DocumentTypeDisplayNameFilter);
			...
			var documentTypeTranslations = from o in pagedAndFilteredDocumentTypeTranslations
                         ...
                         select new GetDocumentTypeTranslationForViewDto() {
							DocumentTypeTranslation = new DocumentTypeTranslationDto
							{
                                Name = o.Name,
                                Language = o.Language,
                                Id = o.Id
		 ...
		 public async Task<GetDocumentTypeTranslationForViewDto> GetDocumentTypeTranslationForView(int id)
         {
            ...
            var output = new GetDocumentTypeTranslationForViewDto { DocumentTypeTranslation = ObjectMapper.Map<DocumentTypeTranslationDto>(documentTypeTranslation) };

		    if (output.DocumentTypeTranslation.CoreId != null)
            {
                var _lookupDocumentType = await _lookup_documentTypeRepository.FirstOrDefaultAsync((long)output.DocumentTypeTranslation.CoreId);
		 ...
		 public async Task<GetDocumentTypeTranslationForEditOutput> GetDocumentTypeTranslationForEdit(EntityDto input)
         {
		    ...
		    var output = new GetDocumentTypeTranslationForEditOutput {DocumentTypeTranslation = ObjectMapper.Map<CreateOrEditDocumentTypeTranslationDto>(documentTypeTranslation)};
