...
namespace TACHYON.Offers
{
    ...
    public class OffersAppService : TACHYONAppServiceBase, IOffersAppService
    {
        ...
        public OffersAppService(IRepository<Offer> offerRepository,
            IOffersExcelExporter offersExcelExporter,
        ...
        protected virtual async Task<PagedResultDto<GetOfferForViewDto>> GetAllPagedResultDto(GetAllOffersInput input)
        {
            var filteredOffers = _offerRepository.GetAll()
                ...
                .WhereIf(!string.IsNullOrWhiteSpace(input.RouteDisplayNameFilter),
                    e => e.RouteFk != null && e.RouteFk.DisplayName == input.RouteDisplayNameFilter);
            ...
            var offers = from o in await pagedAndFilteredOffers.ToListAsync()
                ...
                join o3 in _lookup_goodCategoryRepository.GetAll().Include(x => x.Translations) on o.GoodCategoryId
                ...
                select new GetOfferForViewDto()
                {
                    Offer = new OfferDto
                    {
                        DisplayName = o.DisplayName, Description = o.Description, Price = o.Price, Id = o.Id
                    },
                    ...
                    GoodCategoryDisplayName =
                        ObjectMapper.Map<GoodCategoryDto>(o.GoodCategoryFk)
                            .DisplayName, //s3 == null || s3.DisplayName == null ? "" : s3.DisplayName.ToString(),
        ...
        public async Task<GetOfferForViewDto> GetOfferForView(int id)
        {
            ...
            if (output.Offer.TrucksTypeId != null)
            {
                var _lookupTrucksType =
                    await _lookup_trucksTypeRepository.FirstOrDefaultAsync(output.Offer.TrucksTypeId.Value);
            ...
            if (output.Offer.TrailerTypeId != null)
            {
                var _lookupTrailerType =
                    await _lookup_trailerTypeRepository.FirstOrDefaultAsync((int)output.Offer.TrailerTypeId);
                output.TrailerTypeDisplayName = _lookupTrailerType?.DisplayName?.ToString();
            }

            if (output.Offer.GoodCategoryId != null)
            {
                var _lookupGoodCategory = await _lookup_goodCategoryRepository.GetAllIncluding(x => x.Translations)
                    .FirstOrDefaultAsync(x => x.Id == (int)output.Offer.GoodCategoryId);
        ...
        public async Task<GetOfferForEditOutput> GetOfferForEdit(EntityDto input)
        {
            ...
            if (output.Offer.TrucksTypeId != null)
            {
                var _lookupTrucksType =
                    await _lookup_trucksTypeRepository.FirstOrDefaultAsync(output.Offer.TrucksTypeId.Value);
                output.TrucksTypeDisplayName =
                    ObjectMapper.Map<TrucksTypeDto>(_lookupTrucksType)?.TranslatedDisplayName;
            }
