...
namespace TACHYON.Drivers.importing
{
    public class DriverListExcelDataReader : NpoiExcelImporterBase<ImportDriverDto>, IDriverListExcelDataReader
    {
        ...
        public DriverListExcelDataReader(IRepository<DocumentType, long> documentTypeRepository,
        ...
        private ImportDriverDto ProcessExcelRow(ISheet worksheet, int row)
        {
            ...
            try
            {
                iqamaDocumentType = _documentTypeRepository.GetAll()
                    .First(x => x.SpecialConstant.ToLower() == "DriverIqama".ToLower());
            ...
            try
            {
                drivingLicenseDocumentType = _documentTypeRepository.GetAll()
                    .First(x => x.SpecialConstant.ToLower() == "DriverDrivingLicense".ToLower());
            ...
            try
            {
                occupationCardDocumentType = _documentTypeRepository.GetAll()
                    .First(x => x.SpecialConstant.ToLower() == "DriverOccupationCard".ToLower());
            ...
            {
                //0
                user.Name = _tachyonExcelDataReaderHelper.GetRequiredValueFromRowOrNull<string>(worksheet, row, 0,
                    nameof(user.Name), exceptionMessage);
                //1
                user.Surname =
                    _tachyonExcelDataReaderHelper.GetRequiredValueFromRowOrNull<string>(worksheet, row, 1,
                        nameof(user.Surname), exceptionMessage);
                //2
                user.PhoneNumber =
                    _tachyonExcelDataReaderHelper.GetRequiredValueFromRowOrNull<string>(worksheet, row, 2,
                        nameof(user.PhoneNumber), exceptionMessage);
                ValidatePhoneNumber(user, exceptionMessage);
                //3
                user.EmailAddress = _tachyonExcelDataReaderHelper.GetValueFromRowOrNull<string>(worksheet, row, 3,
                    nameof(user.EmailAddress), exceptionMessage);
                ...
                user.DateOfBirth = _tachyonExcelDataReaderHelper.GetValueFromRowOrNull<DateTime>(worksheet, row, 5,
                    nameof(user.DateOfBirth), exceptionMessage);
                ...
                iqamaDocumentFileDto.HijriExpirationDate =
                    _tachyonExcelDataReaderHelper.GetValueFromRowOrNull<string>(worksheet, row, 7,
                        "ID/Iqama Expiry Date(Hijri)", exceptionMessage);
                //8
                iqamaDocumentFileDto.ExpirationDate =
                    _tachyonExcelDataReaderHelper.GetValueFromRowOrNull<DateTime?>(worksheet, row, 8,
                        "ID/Iqama Expiry Date(Gregorian)", exceptionMessage);
                ...
                drivingLicenseDocumentFileDto.HijriExpirationDate =
                    _tachyonExcelDataReaderHelper.GetValueFromRowOrNull<string>(worksheet, row, 10,
                        "Driving License Expiry Date(Hijri)", exceptionMessage);
                //11
                drivingLicenseDocumentFileDto.ExpirationDate =
                    _tachyonExcelDataReaderHelper.GetValueFromRowOrNull<DateTime?>(worksheet, row, 11,
                        "Driving License Expiry Date(Gregorian)", exceptionMessage);
                ValidateDrivingLicenseDocumentFileDto(exceptionMessage, drivingLicenseDocumentFileDto,
                    drivingLicenseDocumentType);
                ...
                //12
                occupationCardDocumentFileDto.Number =
                    _tachyonExcelDataReaderHelper.GetValueFromRowOrNull<string>(worksheet, row, 12,
                        "Occupation Card NO", exceptionMessage);
        ...
        private void ValidatePhoneNumber(ImportDriverDto user, StringBuilder exceptionMessage)
        {
            ...
            {
                exceptionMessage.Append("PhoneNumber should be " + UserConsts.DriverPhoneNumberLength.ToString() +
                                        " character;");
            }
        }

        private void ValidateIqamaDocumentFileDto(StringBuilder exceptionMessage,
            CreateOrEditDocumentFileDto iqamaDocumentFileDto,
            DocumentType iqamaDocumentType)
        {
        ...
        private void ValidateDrivingLicenseDocumentFileDto(StringBuilder exceptionMessage,
            CreateOrEditDocumentFileDto drivingLicenseDocumentFileDto,
            DocumentType drivingLicenseDocumentType)
        {
