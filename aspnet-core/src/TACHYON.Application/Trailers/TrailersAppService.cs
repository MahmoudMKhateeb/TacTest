...
namespace TACHYON.Trailers
{
    ...
    public class TrailersAppService : TACHYONAppServiceBase, ITrailersAppService
    {
        ...
        public TrailersAppService(IRepository<Trailer, long> trailerRepository,
            ITrailersExcelExporter trailersExcelExporter,
            IRepository<TrailerStatus, int> lookup_trailerStatusRepository,
            IRepository<TrailerType, int> lookup_trailerTypeRepository,
            IRepository<PayloadMaxWeight, int> lookup_payloadMaxWeightRepository,
            IRepository<Truck, long> lookup_truckRepository)
        {
        ...
        public async Task<PagedResultDto<GetTrailerForViewDto>> GetAll(GetAllTrailersInput input)
        {
            var filteredTrailers = _trailerRepository.GetAll()
                ...
                .WhereIf(!string.IsNullOrWhiteSpace(input.Filter),
                    e => false || e.TrailerCode.Contains(input.Filter) || e.PlateNumber.Contains(input.Filter) ||
                         e.Model.Contains(input.Filter) || e.Year.Contains(input.Filter))
                .WhereIf(!string.IsNullOrWhiteSpace(input.TrailerCodeFilter),
                    e => e.TrailerCode == input.TrailerCodeFilter)
                ...
                .WhereIf(input.IsLiftgateFilter > -1,
                    e => (input.IsLiftgateFilter == 1 && e.IsLiftgate) ||
                         (input.IsLiftgateFilter == 0 && !e.IsLiftgate))
                .WhereIf(input.IsReeferFilter > -1,
                    e => (input.IsReeferFilter == 1 && e.IsReefer) || (input.IsReeferFilter == 0 && !e.IsReefer))
                .WhereIf(input.IsVentedFilter > -1,
                    e => (input.IsVentedFilter == 1 && e.IsVented) || (input.IsVentedFilter == 0 && !e.IsVented))
                .WhereIf(input.IsRollDoorFilter > -1,
                    e => (input.IsRollDoorFilter == 1 && e.IsRollDoor) ||
                         (input.IsRollDoorFilter == 0 && !e.IsRollDoor))
                .WhereIf(!string.IsNullOrWhiteSpace(input.TrailerStatusDisplayNameFilter),
                    e => e.TrailerStatusFk != null &&
                         e.TrailerStatusFk.DisplayName == input.TrailerStatusDisplayNameFilter)
                .WhereIf(!string.IsNullOrWhiteSpace(input.TrailerTypeDisplayNameFilter),
                    e => e.TrailerTypeFk != null && e.TrailerTypeFk.DisplayName == input.TrailerTypeDisplayNameFilter)
                .WhereIf(!string.IsNullOrWhiteSpace(input.PayloadMaxWeightDisplayNameFilter),
                    e => e.PayloadMaxWeightFk != null &&
                         e.PayloadMaxWeightFk.DisplayName == input.PayloadMaxWeightDisplayNameFilter)
                .WhereIf(!string.IsNullOrWhiteSpace(input.TruckPlateNumberFilter),
                    e => e.HookedTruckFk != null && e.HookedTruckFk.PlateNumber == input.TruckPlateNumberFilter);
            ...
            var trailers = from o in pagedAndFilteredTrailers
                ...
                select new GetTrailerForViewDto()
                {
                    ...
                    TrailerStatusDisplayName =
                        s1 == null || s1.DisplayName == null ? "" : s1.DisplayName.ToString(),
                    TrailerTypeDisplayName = s2 == null || s2.DisplayName == null ? "" : s2.DisplayName.ToString(),
                    PayloadMaxWeightDisplayName =
                        s3 == null || s3.DisplayName == null ? "" : s3.DisplayName.ToString(),
        ...
        public async Task<GetTrailerForViewDto> GetTrailerForView(long id)
        {
            ...
            {
                var _lookupTrailerStatus =
                    await _lookup_trailerStatusRepository.FirstOrDefaultAsync((int)output.Trailer.TrailerStatusId);
                output.TrailerStatusDisplayName = _lookupTrailerStatus?.DisplayName?.ToString();
            }

            if (output.Trailer.TrailerTypeId != null)
            {
                var _lookupTrailerType =
                    await _lookup_trailerTypeRepository.FirstOrDefaultAsync((int)output.Trailer.TrailerTypeId);
                output.TrailerTypeDisplayName = _lookupTrailerType?.DisplayName?.ToString();
            }

            if (output.Trailer.PayloadMaxWeightId != null)
            {
                var _lookupPayloadMaxWeight =
                    await _lookup_payloadMaxWeightRepository.FirstOrDefaultAsync(
                        (int)output.Trailer.PayloadMaxWeightId);
            ...
            {
                var _lookupTruck =
                    await _lookup_truckRepository.FirstOrDefaultAsync((long)output.Trailer.HookedTruckId);
        ...
        public async Task<List<TrailerTrailerStatusLookupTableDto>> GetAllTrailerStatusForTableDropdown()
        {
            return await _lookup_trailerStatusRepository.GetAll()
                ...
                {
                    Id = trailerStatus.Id,
                    DisplayName = trailerStatus == null || trailerStatus.DisplayName == null
                        ? ""
                        : trailerStatus.DisplayName.ToString()
                }).ToListAsync();
        }
        ...
        public async Task<List<TrailerPayloadMaxWeightLookupTableDto>> GetAllPayloadMaxWeightForTableDropdown()
        {
            return await _lookup_payloadMaxWeightRepository.GetAll()
                .Select(payloadMaxWeight => new TrailerPayloadMaxWeightLookupTableDto
                {
                    ...
                    DisplayName = payloadMaxWeight == null || payloadMaxWeight.DisplayName == null
                        ? ""
                        : payloadMaxWeight.DisplayName.ToString()
                }).ToListAsync();
        }
