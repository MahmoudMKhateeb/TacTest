...
namespace TACHYON.Dates
{
    public static class DateHeleper
    {
        public static List<KeyValuePair> WeeksDayNamesList()
        {
            ...
            for (int i = 0; i <= 6; i++)
            {
                Weeks.Add(new KeyValuePair
                {
                    Value = i, Key = System.Globalization.CultureInfo.CurrentCulture.DateTimeFormat.DayNames[i]
                });
            }
        ...
        public static List<KeyValuePair> MonthNamesList()
        {
            ...
            for (int i = 0; i <= 11; i++)
            {
                Months.Add(new KeyValuePair
                {
                    Key = System.Globalization.CultureInfo.CurrentCulture.DateTimeFormat.MonthNames[i],
                    Value = i + 1
                });
            }
