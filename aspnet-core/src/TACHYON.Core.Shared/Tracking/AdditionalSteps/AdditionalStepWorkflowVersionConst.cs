namespace TACHYON.Tracking.AdditionalSteps
{
    public static class AdditionalStepWorkflowVersionConst
    {
        ...
        /// Ports Movement - Export -> One way with port shuttling/ Two way Route with port shuttling  -> `One Trip Only, Third Trip`
        public const int PortsMovementExportThirdTripVersion = 5;

        // WithoutReceiverCode
        /// Ports Movement - Import -> With/out Return Trip -> `First Trip`
        public const int PortsMovementImportFirstTripWithoutReceiverCode = 6;
        /// Ports Movement - Import -> Without Return Trip -> `Return Trip`
        public const int PortsMovementImportReturnTripWithoutReceiverCode = 7;
        /// Ports Movement - Export -> Two way Route with/out port shuttling -> `First Trip`
        public const int PortsMovementExportFirstTripWithoutReceiverCode = 8;
        /// Ports Movement - Export -> Two way Route with/out port shuttling -> `Second Trip`
        public const int PortsMovementExportSecondTripWithoutReceiverCode = 9;
        /// Ports Movement - Export -> One way with port shuttling/ Two way Route with port shuttling  -> `One Trip Only, Third Trip`
        public const int PortsMovementExportThirdTripWithoutReceiverCode = 10;
